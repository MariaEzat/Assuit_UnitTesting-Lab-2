...
namespace CarFactoryAPI_test
{
    ...
    string result = ownersService.BuyCar(buyCarInput);
        ...
        [Fact]
        public void BuyCar_somthingWrong() {

            outputHelper.WriteLine("Test 6");
            // Arrange

            // Setup the called Methods
            carRepoMock.Setup(cm => cm.GetCarById(It.IsAny<int>())).Throws<Exception>();

            BuyCarInput buyCarInput = new() { CarId = 5, OwnerId = 100, Amount = 10000 }; 
        ...
        [Fact]
        public void BuyCar_Success()
        {
            outputHelper.WriteLine("Test 7");

            // Arrange
            // Build the mock Data
            Car car = new Car() { Id = 5, Price = 5000 }; // Car price is 5000
            Owner owner = new Owner();

            // Setup the called Methods
            carRepoMock.Setup(cm => cm.GetCarById(It.IsAny<int>())).Returns(car);
            ownersRepoMock.Setup(om => om.GetOwnerById(It.IsAny<int>())).Returns(owner);
            cashServiceMock.Setup(cs => cs.Pay(It.IsAny<double>())).Returns("Payment successful"); // Assume payment is successful

            BuyCarInput buyCarInput = new() { CarId = 5, OwnerId = 100, Amount = 5000 }; // Assuming amount is sufficient

            // Act 
            string result = ownersService.BuyCar(buyCarInput);

            // Assert
            Assert.Contains("Successfull", result); // Check if the result contains the success message
        }
